...
void main() {
  ...
  // Pattern 8
  // float strength = mod(vUv.y * 10.0, 1.0);
  // strength = step(0.5, strength); // Same as: strength = strength < 0.5 ? 0.0 : 1.0;

  // Pattern 9
  // float strength = mod(vUv.y * 10.0, 1.0);
  // strength = step(0.8, strength); // Same as: strength = strength < 0.5 ? 0.0 : 1.0;
  ...
  // Pattern 27
  // float strength = distance(vUv, vec2(0.5)); // same as: length(vUv - 0.5)
  ...
  // Pattern 32
  // vec2 rotatedUv = rotate(vUv, PI_4, vec2(0.5));
  ...
  // angle /= PI * 2.0;
  // angle += 0.5;
  // float sinusoid = sin(angle * 100.0) / 50.0;
