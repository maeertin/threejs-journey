...
// Fog
const fog = new THREE.Fog(options.fogColor, 1, 15)
scene.fog = fog
...
const walls = new THREE.Mesh(
  new THREE.BoxGeometry(options.wallsWidth, options.wallsHeight, options.wallsDepth),
...
walls.position.y = options.wallsHeight / 2
house.add(walls)

const roof = new THREE.Mesh(
  new THREE.ConeGeometry(options.roofRadius, options.roofHeight, options.roofRadialSegments),
  new THREE.MeshStandardMaterial({ color: '#b35f45' }),
)
roof.position.y = options.roofHeight / 2 + options.wallsHeight
roof.rotation.y = Math.PI * 0.25
house.add(roof)

const door = new THREE.Mesh(
  new THREE.PlaneGeometry(options.doorWidth, options.doorHeight, 100, 100),
  new THREE.MeshStandardMaterial({
...
door.position.y = options.doorHeight / 2 - 0.1 // Due to alphaMap
door.position.z = options.wallsDepth / 2 - 0.025 // Due to displacementMap
house.add(door)
...
renderer.setSize(sizes.width, sizes.height)
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
renderer.setClearColor(options.fogColor)
